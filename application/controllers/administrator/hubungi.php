...
class Hubungi extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM hubungi ORDER BY id_hubungi";		
			...
			$text = "SELECT * FROM hubungi ORDER BY id_hubungi DESC LIMIT $limit OFFSET $offset";
			$d['data'] = $this->app_model->manualQuery($text);			
			
			
			$d['content'] = $this->load->view('administrator/v_hubungi', $d, true);		
	...
	public function balas()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM hubungi WHERE id_hubungi='$id' LIMIT 1";
			$identitas = $this->app_model->manualQuery($text);
			foreach($identitas->result() as $db){
				$d['id_hubungi']=$db->id_hubungi;
				$d['email_hubungi']=$db->email;
				$d['subjek']=$db->subjek;
				$d['msg']=$db->pesan;
			}
			...
			$d['content'] = $this->load->view('administrator/tambah_hubungi', $d, true);		
			$this->load->view('administrator/home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(4);
			$this->app_model->manualQuery("DELETE FROM hubungi WHERE id_hubungi='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/administrator/hubungi'>";			
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$this->load->library('email');
				 ...
				 $this->email->from($email, $this->session->userdata['username']);
				 $this->email->to($email_hubungi); 
			
				 $this->email->subject('Jawaban '+$website+' : '+$subjek);
				...
				$up['balas'] = 'Y';
				$up['tgl_balas'] = date('Y-m-d');
				
				$id= $this->input->post('id');
				$key['id_hubungi'] =$id;
				$this->app_model->updateData("hubungi",$up,$key);
				
				$d['pesan']='Simpan Sukses';		
				header('location:'.base_url().'index.php/administrator/hubungi');
