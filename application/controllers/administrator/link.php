...
class Link extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM banner ORDER BY id_banner";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$config['base_url'] = site_url() . '/administrator/link/index/';
			...
			$text = "SELECT * FROM banner ORDER BY id_banner DESC LIMIT $limit OFFSET $offset";
			$d['data'] = $this->app_model->manualQuery($text);
			
			
			$d['content'] = $this->load->view('administrator/v_link', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('administrator/tambah_link', $d, true);		
			$this->load->view('administrator/home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM banner WHERE id_banner='$id' LIMIT 1";
			$data = $this->app_model->manualQuery($text);
			foreach($data->result() as $db){
				$d['id_link']=$db->id_banner;
				$d['judul_link']=$db->judul;
				$d['url']=$db->url;
				$d['gambar_link']=$db->gambar;
			}
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(4);
			$this->app_model->manualQuery("DELETE FROM banner WHERE id_banner='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/administrator/link'>";			
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function simpan()
	{
		...
		$cek = $this->session->userdata('logged_in');
		if(!empty($cek)){
			$config['upload_path'] = './asset/foto_banner/';   //base_url().'asset/images/'; //
			...
			if(empty($_FILES['gambar']['name'])){	
				$up['judul'] = $this->input->post('judul_link');
				$up['url'] = $this->input->post('url');
				$up['tgl_posting'] = date('Y-m-d');

				$id= $this->input->post('id');
				if(empty($id)){
					$this->app_model->insertData("banner",$up);
				}else{
					$key['id_banner'] =$id;
					$this->app_model->updateData("banner",$up,$key);
				}
				
				$d['pesan']='Simpan Sukses';		
				header('location:'.base_url().'index.php/administrator/link');
			}else{
