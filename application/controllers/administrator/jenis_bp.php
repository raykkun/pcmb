...
class Jenis_bp extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$config['base_url'] = site_url() . '/administrator/jenis_bp/index/';
			...
			$text = "SELECT * FROM jenis_bahan_pokok ORDER BY id_jenis_bp DESC LIMIT $limit OFFSET $offset";
			$d['data'] = $this->app_model->manualQuery($text);
			
			
			$d['content'] = $this->load->view('administrator/v_jenis_bp', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('administrator/tambah_jenis_bp', $d, true);		
			$this->load->view('administrator/home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM jenis_bahan_pokok WHERE id_jenis_bp='$id' LIMIT 1";
			$data = $this->app_model->manualQuery($text);
			foreach($data->result() as $db){
				$d['id_jenis']=$db->id_jenis_bp;
				$d['jenis']=$db->jenis_bp;
			}
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(4);
			$this->app_model->manualQuery("DELETE FROM jenis_bahan_pokok WHERE id_jenis_bp='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/administrator/jenis_bp'>";			
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['jenis_bp'] = $this->input->post('jenis');


				$id= $this->input->post('id');
				if(empty($id)){
					$this->app_model->insertData("jenis_bahan_pokok",$up);
				}else{
					$key['id_jenis_bp'] =$id;
					$this->app_model->updateData("jenis_bahan_pokok",$up,$key);
				}
				
				$d['pesan']='Simpan Sukses';		
				header('location:'.base_url().'index.php/administrator/jenis_bp');
