...
class Kepala extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM pimpinan WHERE id='1' LIMIT 1";
			$kepala = $this->app_model->manualQuery($text);
			foreach($kepala->result() as $db){
				$d['nama_kepala'] = $db->nama;
				$d['nip'] = $db->nip;
				$d['foto'] = $db->foto;
				$d['ket'] = $db->profil;
			}
			
			$d['judul'] = "Kepala Kantor";
			$d['pesan']='';
			
			$d['scriptmce'] = $this->scripttiny_mce();
			$d['content'] = $this->load->view('administrator/kepala', $d, true);		
	...
	public function simpan()
	{
		...
		$cek = $this->session->userdata('logged_in');
		if(!empty($cek)){
			$config['upload_path'] = base_url().'asset/images/';
