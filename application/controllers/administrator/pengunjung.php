...
class Pengunjung extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			/*
			$text = "SELECT month(tanggal) as bulan, year(tanggal) as th, count(*) as total FROM statistik GROUP BY month(tanggal),year(tanggal)";
			$d['data'] = $this->app_model->manualQuery($text);
			*/
			
			
			$d['content'] = $this->load->view('administrator/v_pengunjung', $d, true);		
