...
function simpan(){
	var KerjaOrtu = $("#KdKerjaOrtu").val();
	
	var string = $("#form").serialize();
	
	//alert(string);
	
	if(KerjaOrtu.length==0){
		$.messager.show({
			title:'Info',
			msg:'Maaf, Pekerjaan Ortu tidak boleh kosong',
			timeout:2000,
			showType:'slide'
		});
		$("#KdKerjaOrtu").focus();
	...
	$.ajax({
		type	: "POST",
		url		: "<?php echo site_url('administrator/pekerjaan_ortu/simpan'); ?>",
		data	: string,
	...
	if (row){  
		$.messager.confirm('Confirm','Apakah Anda akan menghapus data ini ?',function(r){  
			if (r){  
				$.ajax({
					type	: "POST",
					url		: "<?php echo site_url('administrator/pekerjaan_ortu/hapus'); ?>",
					data	: 'id='+row.KdKerjaOrtu,
					success	: function(data){
						$.messager.show({
							title:'Info',
							msg:data,
...
<!-- Data Grid -->
<table id="datagrid-crud" title="Daftar <?php echo $judul;?>" class="easyui-datagrid" style="width:auto; height: auto;" url="<?php echo site_url('administrator/pekerjaan_ortu/view'); ?>?grid=true" toolbar="#toolbar" pagination="true" rownumbers="true" fitColumns="true" singleSelect="true" collapsible="true">
	<thead>
		<tr>
			<th field="KdKerjaOrtu" width="10" sortable="true">Kode</th>
			<th field="KerjaOrtu" width="50" sortable="true">Pekerjaan</th>
		</tr>
	</thead>
</table>
...
<div id="dialog-form" class="easyui-dialog" style="width:500px; height:auto; padding: 20px 30px" closed="true" buttons="#dialog-buttons">
	<form id="form" method="post" novalidate>
		...
		<div class="form-item">
			<label for="type">Pekerjaan</label><br />
