...
class Header extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM header ORDER BY id_header";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$config['base_url'] = site_url() . '/administrator/header/index/';
			...
			$text = "SELECT * FROM header ORDER BY id_header DESC LIMIT $limit OFFSET $offset";
			$d['data'] = $this->app_model->manualQuery($text);
			
			
			$d['content'] = $this->load->view('administrator/v_header', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('administrator/tambah_header', $d, true);		
			$this->load->view('administrator/home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM header WHERE id_header='$id' LIMIT 1";
			$data = $this->app_model->manualQuery($text);
			foreach($data->result() as $db){
				$d['id_header']=$db->id_header;
				$d['judul_header']=$db->judul;
				$d['status']=$db->status;
				$d['gambar_header']=$db->gambar;
			}
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(4);
			$this->app_model->manualQuery("DELETE FROM header WHERE id_header='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/administrator/header'>";			
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function simpan()
	{
		...
		$cek = $this->session->userdata('logged_in');
		if(!empty($cek)){
			$config['upload_path'] = './asset/slider/';   //base_url().'asset/images/'; //
			$config['allowed_types'] = 'bmp|gif|jpg|jpeg|png';
			$config['max_size'] = '1000';
			$config['max_width'] = '972';
			$config['max_height'] = '250';						
			$this->load->library('upload', $config);
			
			if(empty($_FILES['gambar']['name'])){	
				$up['judul'] = $this->input->post('judul_header');
				$up['status'] = $this->input->post('status');

				$id= $this->input->post('id');
				if(empty($id)){
					$this->app_model->insertData("header",$up);
				}else{
					$key['id_header'] =$id;
					$this->app_model->updateData("header",$up,$key);
				}
				
				$d['pesan']='Simpan Sukses';		
				header('location:'.base_url().'index.php/administrator/header');
			}else{
