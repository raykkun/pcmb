...
class Ym extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM ym ORDER BY id_ym";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$config['base_url'] = site_url() . '/administrator/ym/index/';
			...
			$text = "SELECT * FROM ym ORDER BY id_ym DESC LIMIT $limit OFFSET $offset";
			$d['data'] = $this->app_model->manualQuery($text);
			
			
			$d['content'] = $this->load->view('administrator/v_ym', $d, true);		
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('administrator/tambah_ym', $d, true);		
			$this->load->view('administrator/home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM ym WHERE id_ym='$id' LIMIT 1";
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			$id = $this->uri->segment(4);
			$this->app_model->manualQuery("DELETE FROM ym WHERE id_ym='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/administrator/ym'>";			
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function simpan()
	{
		...
		if(!empty($cek)){
				...
				$up['nama'] = $this->input->post('nama_ym');
				$up['ym'] = $this->input->post('ym');

				$id= $this->input->post('id');
				if(empty($id)){
					$this->app_model->insertData("ym",$up);
				}else{
					$key['id_ym'] =$id;
					$this->app_model->updateData("ym",$up,$key);
				}
				
				$d['pesan']='Simpan Sukses';		
				header('location:'.base_url().'index.php/administrator/ym');
