...
class Pasar extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM pasar ORDER BY id_pasar";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$config['base_url'] = site_url() . '/administrator/pasar/index/';
			...
			$text = "SELECT * FROM pasar ORDER BY id_pasar LIMIT $limit OFFSET $offset";
			...
			$d['content'] = $this->load->view('administrator/v_pasar', $d, true);		
	...
	public function filter()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM pasar $where ORDER BY id_pasar";		
			$tot_hal = $this->app_model->manualQuery($text);		
			
			$config['base_url'] = site_url() . '/administrator/pasar/filter/';
			$config['total_rows'] = $tot_hal->num_rows();
			$config['per_page'] = $limit;
			$config['uri_segment'] = 4;
			$config['next_link'] = 'Lanjut &raquo;';
			...
			$text = "SELECT * FROM pasar $where ORDER BY id_pasar LIMIT $limit OFFSET $offset";
			$d['data'] = $this->app_model->manualQuery($text);
	...
	public function tambah()
	{
		...
		if(!empty($cek)){
			...
			$d['content'] = $this->load->view('administrator/tambah_pasar', $d, true);		
			$this->load->view('administrator/home',$d);
		}else{
			header('location:'.base_url());
		}
	...
	public function edit()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM pasar WHERE id_pasar='$id' LIMIT 1";
			$data = $this->app_model->manualQuery($text);
			foreach($data->result() as $t){
				$d['id_pasar'] =$t->id_pasar;
				$d['id_kota'] =$t->id_kota;
				$d['id_kec'] =$t->id_kec;
				$d['kecamatan'] =$this->app_model->kec($t->id_kec);
				$d['pasar'] =$t->nama_pasar;
				$d['ket'] =$t->ket;
				$d['foto'] =$t->foto;
			}
	...
	public function hapus()
	{
		...
		if(!empty($cek)){			
			...
			$this->app_model->manualQuery("DELETE FROM pasar WHERE id_pasar='$id'");
			echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/administrator/pasar'>";			
		}else{
			header('location:'.base_url());
		}
	}
	...
	public function simpan()
	{
		...
		$cek = $this->session->userdata('logged_in');
		if(!empty($cek)){
			$config['upload_path'] = './asset/pasar/';   //base_url().'asset/images/'; //
			...
			$up['nama_pasar'] = $this->input->post('pasar');
			...
			if(empty($_FILES['foto']['name'])){	
				$up['foto'] = 'kosong.jpg';
				if(empty($id)){
					$this->app_model->insertData("pasar",$up);
				}else{
					$key['id_pasar'] =$id;
					$this->app_model->updateData("pasar",$up,$key);
				}
				
				$d['pesan']='Simpan Sukses';		
				header('location:'.base_url().'index.php/administrator/pasar');
			}else{
