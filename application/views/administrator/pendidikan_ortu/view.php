...
function save(){
	...
	$.ajax({
		type	: "POST",
		url		: "<?php echo site_url('administrator/pendidikan_ortu/simpan'); ?>",
		data	: string,
	...
	if (row){  
		$.messager.confirm('Confirm','Apakah Anda akan menghapus data ini ?',function(r){  
			if (r){  
				$.ajax({
					type	: "POST",
					url		: "<?php echo site_url('administrator/pendidikan_ortu/hapus'); ?>",
					data	: 'id='+row.KdPendOrtu,
					success	: function(data){
						$.messager.show({
							title:'Info',
							msg:data,
...
<!-- Data Grid -->
<table id="datagrid-crud" title="Daftar <?php echo $judul;?>" class="easyui-datagrid" style="width:auto; height: auto;" url="<?php echo site_url('administrator/pendidikan_ortu/view'); ?>?grid=true" toolbar="#toolbar" pagination="true" rownumbers="true" fitColumns="true" singleSelect="true" collapsible="true">
	<thead>
		<tr>
			<th field="KdPendOrtu" width="10" sortable="true">Kode</th>
			<th field="Pendidikan" width="50" sortable="true">Pendidikan</th>
		</tr>
	</thead>
</table>
...
<div id="dialog-form" class="easyui-dialog" style="width:500px; height:auto; padding: 20px 30px" closed="true" buttons="#dialog-buttons">
	<form id="form" method="post" novalidate>
		<div class="form-item">
			...
			<input type="text" name="KdPendOrtu" id="KdPendOrtu" class="easyui-validatebox" required="true" size="30" maxlength="30"  />
		</div>
		<div class="form-item">
			<label for="type">Jenjang Pendidikan</label><br />
			<input type="text" name="Pendidikan" id="Pendidikan" class="easyui-validatebox" required="true" size="50" maxlength="50" />
		</div>
	</form>
</div>
