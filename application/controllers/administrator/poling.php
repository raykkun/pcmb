...
class Poling extends CI_Controller {
	...
	public function index()
	{
		...
		if(!empty($cek)){
			...
			$text = "SELECT * FROM poling WHERE status='Pertanyaan'";
			$d['polling_tanya'] = $this->app_model->manualQuery($text);
			
			$text = "SELECT * FROM poling WHERE status='Jawaban'";
			$d['polling_hasil'] = $this->app_model->manualQuery($text);
			
			
			$d['content'] = $this->load->view('administrator/v_poling', $d, true);		
